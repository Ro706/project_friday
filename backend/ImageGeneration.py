...
async def query(payload):
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"API Info {response.status_code}: Model is loading. Retrying in 10 seconds...")
        ...
        print(f"Error during image generation request: {e}")
        return b''
...
def open_image(prompt):
    ...
    if not found_any:
        print(f"No images were successfully generated for prompt: {prompt}")
